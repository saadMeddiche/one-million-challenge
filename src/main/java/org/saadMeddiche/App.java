...
public class App 
{
    ...
    public static final String FILE_NAME_TO_READ_FROM = "one-million-challenge_sample";
    public static final String FILE_NAME_TO_WRITE_INTO = "one-million-challenge";

    private static final int SCRIPT_MOD = 1; // == 0 -> generation ; != 0 -> extraction

    private static final TxtFileGenerator GENERATOR = new SeekableByteChannelTxtFileGenerator();
    private static final TxtFileExtractor EXTRACTOR = new EmptyStreamTxtFileExtractor();
    ...
    public static void extraction_script() {
        ...
        if(extractionResult.success()) {
            System.out.printf("├SCRIPT: extraction_script #success #sum(%s$%b)",
                    extractionResult.totalSum(), -15673375262273L == extractionResult.totalSum());
        } else {
